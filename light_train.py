...
def parse_args():
    ...
    parser.add_argument('--out_dir', type=str, default="output_new", help='output_dir')
...
def train():
    ...
    # model = DARPA_DET.load_from_checkpoint("/media/jiahua/FILE/uiuc/NCSA/DARPA_torch/exp/lightning_logs/version_363/checkpoints/epoch=14-step=16305.ckpt",args=args)
    train_dataset1 = DetData(data_path=args.train_data,type="point",range = train_range, args=args)
    # val_dataset = GENData(data_path=args.train_data,type="point", args=args)
    train_dataset2 = GENData(data_path=args.val_data,type="point", args=args)
    train_dataset = train_dataset1
    # val_dataset = train_dataset2
    # val_dataset = GENData(data_path=args.val_data,type="point", args=args)
    val_dataset = DetData(data_path=args.val_data,type="point",range = val_range, args=args)
    # train_dataset[0]
    # exit(0)
